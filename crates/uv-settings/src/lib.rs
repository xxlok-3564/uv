...
mod test {
    ...
    fn test_locate_system_config_xdg() -> Result<(), FixtureError> {
        ...
        let first_td = context.child("first");
        let first_tf = first_td.child("uv").child("uv.toml");
        first_tf.write_str("")?;
    ...
    fn test_windows_config() -> Result<(), FixtureError> {
        ...
        env::set_var(EnvVars::SYSTEMDRIVE, context.path().clone());
        assert_eq!(
